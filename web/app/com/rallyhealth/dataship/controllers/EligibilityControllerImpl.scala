...
class EligibilityControllerImpl(
  memPEService: MemPEService,
  optumClientConfig: OptumClientConfig)(implicit ec: ExecutionContext, neoEncryptionService: NeoEncryptionService) extends EligibilityController with DefaultLogger {
  def getEligibility: EssentialAction = Action.async(parse.json) { request =>
